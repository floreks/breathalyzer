...
func (this *MCP342X) debugRead(buf []byte) {
	log.Printf("Read from the device %v", buf)
}
...
func main() {
	mcp3424 := NewMCP342X(MCP3424_ADDRESS, MCP3424_DEFAULT_BUS, MCP342X_GAIN_X1, MCP342X_12_BIT)

	err := mcp3424.init(MCP342X_MODE_ONE_SHOOT)
	if err != nil {
	...
	err = mcp3424.startConversion(MCP342X_CHANNEL_4)
	if err != nil {
		log.Fatalf(err.Error())
	}
